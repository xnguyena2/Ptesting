...
public interface StatisticTotalBenifitRepository extends ReactiveCrudRepository<BenifitByMonth, Long> {
    ...
    @Query(value = "SELECT COALESCE(SUM(payment), 0) AS revenue, COALESCE(SUM(discount_amount + discount_percent / 100 * price), 0) AS discount, COUNT(*) AS count, 0 AS profit, 0 AS cost, 0 AS price, 0 AS ship_price, 0 AS buyer FROM user_package_detail WHERE user_package_detail.group_id = :group_id AND user_package_detail.device_id = :device_id AND user_package_detail.status = :status AND (user_package_detail.createat AT TIME ZONE '+07' BETWEEN :fromtime AND :totime)")
    Mono<BenifitByMonth> getTotalStatictisOfBuyer(@Param("group_id") String groupID, @Param("device_id") String device_id, @Param("fromtime") Timestamp from, @Param("totime") Timestamp to, @Param("status") UserPackageDetail.Status status);
}
