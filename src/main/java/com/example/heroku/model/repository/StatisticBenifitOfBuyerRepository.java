...
public interface StatisticBenifitOfBuyerRepository extends ReactiveCrudRepository<BenifitByBuyer, Long> {
    @Query(value = "SELECT ss.count, ss.revenue, ss.profit, buyer.reciver_fullname AS name, buyer.phone_number AS id FROM (SELECT COUNT(*) AS COUNT, COALESCE(SUM(payment), 0) AS revenue, COALESCE(SUM(profit), 0) AS profit, device_id AS device_id FROM user_package_detail WHERE user_package_detail.group_id = :group_id AND user_package_detail.status = :status AND (user_package_detail.createat BETWEEN :fromtime AND :totime) GROUP BY device_id LIMIT :size OFFSET (:page * :size)) AS ss LEFT JOIN (SELECT * FROM buyer WHERE buyer.group_id = :group_id) AS buyer ON ss.device_id = buyer.device_id")
