...
public interface VoucherRepository extends ReactiveCrudRepository<Voucher, String> {
    ...
    @Query(value = "SELECT v.id, v.voucher_second_id, v.detail, v.discount, v.amount, CASE WHEN v_d.reuse > v.reuse THEN v_d.reuse ELSE v.reuse END AS REUSE, v.for_all_beer, v.for_all_user, v.date_expire, v.status, v.createat FROM (SELECT voucher.* FROM voucher LEFT JOIN voucher_relate_beer ON voucher.voucher_second_id = voucher_relate_beer.voucher_second_id WHERE voucher.voucher_second_id = :voucher AND (voucher.for_all_beer OR voucher_relate_beer.beer_second_id = :beer)) v LEFT JOIN ( (SELECT :device AS device_id, REUSE, voucher_second_id FROM voucher WHERE EXISTS (SELECT * FROM package_order WHERE phone_number_clean = :device) AND voucher_second_id = 'patrons') UNION (SELECT device_id, REUSE, voucher_second_id FROM voucher_relate_user_device)) v_d ON v.voucher_second_id = v_d.voucher_second_id WHERE v.for_all_user OR v_d.device_id = :device")
