...
public interface VoucherRepository extends ReactiveCrudRepository<Voucher, String> {
    ...
    @Query(value = "SELECT v.id, v.voucher_second_id, v.detail, v.discount, v.amount, case when v.reuse > v_d.reuse then v_d.reuse else v.reuse end as reuse, v.for_all_beer, v.for_all_user, v.date_expire, v.status, v.createat FROM (SELECT voucher.* FROM voucher LEFT JOIN voucher_relate_beer ON voucher.voucher_second_id = voucher_relate_beer.voucher_second_id WHERE voucher.voucher_second_id = :voucher AND (voucher.for_all_beer OR voucher_relate_beer.beer_second_id = :beer)) v LEFT JOIN voucher_relate_user_device v_d ON v.voucher_second_id = v_d.voucher_second_id WHERE v.for_all_user OR v_d.device_id = :device")
