...
public interface BeerUnitRepository extends ReactiveCrudRepository<ProductUnit, Long> {
    ...
    @Query(value = "UPDATE product_unit SET has_component = :has_component, buy_price = :buy_price WHERE product_unit.group_id = :group_id AND product_unit.product_second_id = :product_second_id AND product_unit.product_unit_second_id = :product_unit_second_id")
    Mono<ProductUnit> updateHasComponent(@Param("group_id")String groupID, @Param("product_second_id")String product_second_id, @Param("product_unit_second_id")String product_unit_second_id, @Param("has_component") boolean has_component, @Param("buy_price") float buy_price);
}
