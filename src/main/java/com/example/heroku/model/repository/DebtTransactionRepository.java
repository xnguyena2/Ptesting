...
public interface DebtTransactionRepository extends ReactiveCrudRepository<DebtTransation, Long> {
    ...
    @Query(value = "INSERT INTO debt_transaction( group_id, transaction_second_id, device_id, action_id, action_type, transaction_type, amount, category, money_source, note, status, createat ) VALUES ( :group_id, :transaction_second_id, :device_id, :action_id, :action_type, :transaction_type, :amount, :category, :money_source, :note, :status, :createat ) ON CONFLICT ON CONSTRAINT UQ_group_debt_transaction DO UPDATE SET device_id = :device_id, action_id = :action_id, action_type = :action_type, transaction_type = :transaction_type, amount = :amount, category = :category, money_source = :money_source, note = :note, status = :status, createat = :createat")
