...
public interface StoreManagementRepository extends ReactiveCrudRepository<Store, Long> {
    ...
    @Query(value = "SELECT * FROM store WHERE payment_status IS NOT NULL AND payment_status != 'NOT_PAID'")
    Flux<Store> getAllPaid();

    @Query(value = """
            SELECT DISTINCT store.*
            FROM user_package_detail
            LEFT JOIN store ON store.group_id = user_package_detail.group_id
            WHERE store.payment_status IS NULL AND meta_search like :search AND user_package_detail.status = 'DONE'
            """)
    Flux<Store> searchStoreNotPaidOfDonePackageDetailMetaSearch(@Param("search") String search);
}
