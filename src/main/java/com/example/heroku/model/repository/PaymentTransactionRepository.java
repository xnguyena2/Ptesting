...
public interface PaymentTransactionRepository extends ReactiveCrudRepository<PaymentTransation, Long> {
    ...
    @Query(value = "INSERT INTO payment_transaction( group_id, transaction_second_id, device_id, package_second_id, action_id, action_type, transaction_type, amount, category, money_source, note, status, createat ) VALUES ( :group_id, :transaction_second_id, :device_id, :package_second_id, :action_id, :action_type, :transaction_type, :amount, :category, :money_source, :note, :status, :createat ) ON CONFLICT ON CONSTRAINT UQ_group_payment_transaction DO UPDATE SET device_id = :device_id, package_second_id = :package_second_id, action_id = :action_id, action_type = :action_type, transaction_type = :transaction_type, amount = :amount, category = :category, money_source = :money_source, note = :note, status = :status, createat = :createat")
    ...
    @Query(value = "SELECT * FROM payment_transaction WHERE payment_transaction.group_id = :group_id AND (payment_transaction.createat BETWEEN :fromtime AND :totime)")
