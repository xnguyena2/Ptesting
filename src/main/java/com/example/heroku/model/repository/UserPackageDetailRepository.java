...
public interface UserPackageDetailRepository extends ReactiveCrudRepository<UserPackageDetail, Long> {
    @Query(value = "INSERT INTO user_package_detail(group_id, device_id, staff_id, package_second_id, package_type, area_id, area_name, table_id, table_name, voucher, price, payment, discount_amount, discount_percent, ship_price, cost, profit, note, image, progress, status, createat) VALUES (:group_id, :device_id, :staff_id, :package_second_id, :package_type, :area_id, :area_name, :table_id, :table_name, :voucher, :price, :payment, :discount_amount, :discount_percent, :ship_price, :cost, :profit, :note, :image, :progress, :status, :createat) ON CONFLICT (group_id, device_id, package_second_id) DO UPDATE SET staff_id = :staff_id, package_type = :package_type, area_id = :area_id, area_name = :area_name, table_id = :table_id, table_name = :table_name, voucher = :voucher, price = :price, payment = :payment, discount_amount = :discount_amount, discount_percent = :discount_percent, ship_price = :ship_price, cost = :cost, profit = :profit, note = :note, image = :image, progress = :progress, status = :status")
    Mono<UserPackageDetail> InsertOrUpdate(@Param("group_id")String group_id, @Param("device_id") String device_id, @Param("staff_id") String staff_id,
                                           ...
                                           @Param("ship_price") float ship_price, @Param("cost") float cost, @Param("profit") float profit,
    ...
    @Query(value = "SELECT * FROM user_package_detail WHERE user_package_detail.group_id = :group_id AND user_package_detail.package_second_id = :package_second_id AND user_package_detail.table_id = :table_id AND user_package_detail.status IS DISTINCT FROM :status ORDER BY createat DESC LIMIT 1")
    Mono<UserPackageDetail> GetPackageDetailByIdNotStatus(@Param("group_id") String group_id, @Param("package_second_id") String package_id, @Param("table_id") String table_id, @Param("status") UserPackageDetail.Status status);
