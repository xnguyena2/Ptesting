...
public interface ImageRepository extends ReactiveCrudRepository<Image, Long> {
    ...
    @Query(value = "SELECT image.* FROM (SELECT * FROM product WHERE group_id = :group_id) AS product INNER JOIN (SELECT * FROM image WHERE group_id = :group_id) AS image ON image.category = product.product_second_id AND image.tag IS NULL")//, nativeQuery = true)
    Flux<Image> getAllOfOnlyForProduct(@Param("group_id")String group_id);
    ...
    Flux<Image> getAllOfProductByPackageAferID(@Param("group_id") String group_id, @Param("status") UserPackageDetail.Status status, @Param("or_status") UserPackageDetail.Status or_status, @Param("id") long id, @Param("page") int page, @Param("size") int size);
