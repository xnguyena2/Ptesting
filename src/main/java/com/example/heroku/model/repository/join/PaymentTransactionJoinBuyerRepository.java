...
public interface PaymentTransactionJoinBuyerRepository extends ReactiveCrudRepository<PaymentTransactionJoinBuyer, Long> {
    ...
    @Query(value = """
            SELECT payment_transaction.*,
                   buyer.device_id,
                   buyer.reciver_address,
                   buyer.region_id,
                   buyer.district_id,
                   buyer.ward_id,
                   buyer.reciver_fullname,
                   buyer.phone_number,
                   buyer.phone_number_clean,
                   buyer.total_price,
                   buyer.real_price,
                   buyer.ship_price,
                   buyer.discount,
                   ...
                   buyer.debt_out_come,
                   buyer.meta_search,
                   buyer.status AS buyer_status,
                   buyer.group_id AS buyer_group_id,
                   buyer.createat AS buyer_createat
            FROM payment_transaction
            LEFT JOIN
              (SELECT *
               FROM buyer
               WHERE group_id = :group_id) AS buyer ON payment_transaction.device_id = buyer.device_id
            WHERE payment_transaction.group_id = :group_id
              AND payment_transaction.package_second_id = :package_second_id
            """)
