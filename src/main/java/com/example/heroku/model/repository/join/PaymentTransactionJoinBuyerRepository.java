...
public interface PaymentTransactionJoinBuyerRepository extends ReactiveCrudRepository<PaymentTransactionJoinBuyer, Long> {
    ...
    @Query(value = "SELECT payment_transaction.*, buyer.device_id, buyer.reciver_address, buyer.region_id, buyer.district_id, buyer.ward_id, buyer.reciver_fullname, buyer.phone_number, buyer.phone_number_clean, buyer.total_price, buyer.real_price, buyer.ship_price, buyer.discount, buyer.point, buyer.meta_search, buyer.status, buyer.group_id AS buyer_group_id, buyer.createat AS buyer_createat FROM payment_transaction LEFT JOIN buyer ON payment_transaction.device_id = buyer.device_id WHERE payment_transaction.group_id = :group_id AND (payment_transaction.createat AT TIME ZONE '+07' BETWEEN :fromtime AND :totime)")
    Flux<PaymentTransactionJoinBuyer> getStatictis(@Param("group_id") String groupID, @Param("fromtime") Timestamp from, @Param("totime") Timestamp to);
}
