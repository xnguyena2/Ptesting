...
public interface UserPackageRepository extends ReactiveCrudRepository<UserPackage, Long> {
    ...
    @Query(value = "SELECT * FROM user_package WHERE user_package.group_id = :group_id AND user_package.status = :status AND user_package.product_second_id = :product_second_id AND user_package.product_unit_second_id = :product_unit_second_id AND (user_package.createat BETWEEN :fromtime AND :totime) LIMIT :size OFFSET (:page * :size)")
    ...
    @Query(value = "INSERT INTO user_package(group_id, device_id, package_second_id, product_second_id, product_unit_second_id, product_name, product_unit_name, product_group_unit_name, number_services_unit, number_unit, status, depend_to_product, list_product_serial_id, createat) (SELECT :group_id, :device_id, :package_id, :product_second_id, :product_unit_second_id, :product_name, :product_unit_name, :product_group_unit_name, :number_services_unit, :number_unit, :status, :depend_to_product, :list_product_serial_id, NOW() WHERE EXISTS (SELECT * FROM product WHERE product.product_second_id = :product_second_id AND product.status IS DISTINCT FROM 'SOLD_OUT' AND product.status IS DISTINCT FROM 'HIDE' ) AND EXISTS (SELECT * FROM product_unit WHERE product_unit.product_second_id = :product_second_id AND product_unit.product_unit_second_id = :product_unit_second_id AND product_unit.status IS DISTINCT FROM 'SOLD_OUT' AND product_unit.status IS DISTINCT FROM 'HIDE' ) ) ON CONFLICT ON CONSTRAINT UQ_user_package DO UPDATE SET number_unit = user_package.number_unit + :number_unit")
    ...
    Mono<UserPackage> InsertOrUpdatePackage(@Param("group_id") String groupID, @Param("device_id") String device_id, @Param("package_second_id") String package_second_id,
                                            ...
                                            @Param("product_name") String product_name, @Param("product_unit_name") String product_unit_name, @Param("product_group_unit_name") String product_group_unit_name,
                                            @Param("product_type") String product_type, @Param("number_services_unit") float number_services_unit, @Param("number_unit") float number_unit,
