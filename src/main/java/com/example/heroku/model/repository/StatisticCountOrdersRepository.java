...
public interface StatisticCountOrdersRepository extends ReactiveCrudRepository<CountOrderByDate, Long> {
    ...
    @Query(value = "SELECT COUNT(CASE WHEN \"status\" = :cancel_status THEN 1 END) AS count_cancel, COUNT(status) FILTER (WHERE \"status\" = :return_status) AS count_return FROM user_package_detail WHERE user_package_detail.group_id = :group_id AND (user_package_detail.status = :cancel_status OR user_package_detail.status = :return_status) AND (user_package_detail.createat AT TIME ZONE '+07' BETWEEN :fromtime AND :totime)")
