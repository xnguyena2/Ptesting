    ...
    public Mono<BeerSubmitData> updateProductComponent(@Valid @ModelAttribute BeerInfo info) {
        ...
        return Mono.just(info.getProduct())
                .thenMany(Flux.just(info.getProductUnit())
                        .flatMap(beerUnit ->
                                this.beerUnitRepository.updateHasComponent(beerUnit.getGroup_id(), beerUnit.getProduct_second_id(), beerUnit.getProduct_unit_second_id(), beerUnit.isHas_component(), beerUnit.getPrice(), beerUnit.getBuy_price())
