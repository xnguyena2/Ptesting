...
CREATE TABLE IF NOT EXISTS product (id SERIAL PRIMARY KEY, beer_second_id VARCHAR, name VARCHAR, detail TEXT, category VARCHAR, meta_search TEXT, status VARCHAR, createat TIMESTAMP);
CREATE TABLE IF NOT EXISTS product_unit (id SERIAL PRIMARY KEY, beer_unit_second_id VARCHAR, beer VARCHAR, name VARCHAR, price float8, discount float8, date_expire TIMESTAMP, volumetric float8, weight float8, status VARCHAR, createat TIMESTAMP);
...
CREATE TABLE IF NOT EXISTS product_order (id SERIAL PRIMARY KEY, name VARCHAR, package_order_second_id VARCHAR, beer_second_id VARCHAR, voucher_second_id VARCHAR, total_price float8, ship_price float8, createat TIMESTAMP);
CREATE TABLE IF NOT EXISTS product_unit_order (id SERIAL PRIMARY KEY, name VARCHAR, package_order_second_id VARCHAR, beer_second_id VARCHAR, beer_unit_second_id VARCHAR, price float8, total_discount float8, number_unit INTEGER, createat TIMESTAMP);
...
CREATE INDEX product_index ON product(beer_second_id);
...
ALTER TABLE product ADD CONSTRAINT UQ_product_second_id UNIQUE(beer_second_id);
ALTER TABLE product_unit ADD CONSTRAINT UQ_product_unit_second_id UNIQUE(beer_unit_second_id);
ALTER TABLE search_token ADD CONSTRAINT UQ_search_token_product_second_id UNIQUE(beer_second_id);
