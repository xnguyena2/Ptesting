...
CREATE TABLE IF NOT EXISTS user_package_detail (id SERIAL PRIMARY KEY, group_id VARCHAR NOT NULL, package_second_id VARCHAR, device_id VARCHAR, staff_id VARCHAR, staff_name VARCHAR, package_type VARCHAR, area_id VARCHAR, area_name VARCHAR, table_id VARCHAR, table_name VARCHAR, voucher VARCHAR, price float8, payment float8, discount_amount float8, discount_percent float8, discount_promotional float8, discount_by_point float8, additional_fee float8, additional_config VARCHAR, ship_price float8, deliver_ship_price float8, cost float8, profit float8, point INTEGER, note VARCHAR, image VARCHAR, progress VARCHAR, meta_search VARCHAR, money_source VARCHAR, print_kitchen VARCHAR, update_note VARCHAR, customer_info VARCHAR, status VARCHAR, createat TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
...
--      WE SHOULD RAISE EXCEPTION BUT in case delete user_package it make product_import to status RETURN and when insert back it update and stuck here
    ...
    IF (NEW.status <> 'RETURN' AND (OLD.amount = NEW.amount OR NEW.type = 'UPDATE_NUMBER')) OR NEW.type IN ('DELETE_PRODUCT', 'SELLING', 'SELLING_RETURN')
    THEN
	    RETURN NEW;
    END IF;
